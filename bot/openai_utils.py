...
async def transcribe_audio(audio_file) -> str:
    r = await openai.Audio.transcribe("whisper-1", audio_file)
    return r["text"] or ""


async def generate_images(prompt, n_images=4, size="1024x1024", model="dall-e-3"):
    r = await aclient.images.generate(prompt=prompt, n=n_images, size=size, model=model)
